...
if (!$authenticated) {
  ...
  $_SESSION['CAS'] = true;
  header("Location: https://cas.iu.edu/cas/login?cassvc=appCode&casurl=https:phpcas.test");
  exit;
}
...
if ($authenticated) {
  ...
  if (isset($_GET["casticket"])) {
	...
	$cassvc = 'GUEST';  //search kb.indiana.edu for "cas application code" to determine code to use here in place of "appCode"
	$casurl = 'https://php.test/home2.php'; //same base URLsent
  ...
  else
  {
     ...
     $_SESSION['CAS'] = true;
     header("Location: https://cas.iu.edu/cas/login?cassvc=GUEST&casurl=http://127.0.0.4/index.php");
     exit;
  }
}
?>
